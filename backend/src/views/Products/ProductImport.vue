...
onMounted(() => {
  ...
  setupFileDragDropEvents();
});

onBeforeUnmount(() => {
  // Clean up any intervals if component is unmounted
  if (importInterval.value) {
    clearInterval(importInterval.value);
  }
  
  // Clean up event listeners
  cleanupFileDragDropEvents();
});

// Setup file drag and drop
function setupFileDragDropEvents() {
...
function cleanupFileDragDropEvents() {
  const dropArea = document.querySelector('.border-dashed');
  if (!dropArea) return;
...
function goToReviewStep() {
  if (!isConfigValid.value) {
    store.commit('showToast', {
      type: 'error',
      message: 'Please map the required fields (Product Name and Price)',
...
function navigateToProducts() {
  ...
  store.commit('showToast', {
    type: 'success',
    message: `Successfully imported ${importStats.value.success} products`,
