<template>
  ...
  // Filtered products - only show published
  // Fix for the productsToShow computed property
const productsToShow = computed(() => {
  if (!products.value.data) return [];
  
  // Log products to debug
  console.log("All products:", products.value.data);
  
  // Try different ways to check if a product is published
  return products.value.data.filter(p => {
    // Convert the published property to a boolean in various ways
    const isPublished = normalizePublished(p.published) || 
                        p.published === true || 
                        p.published === 1 || 
                        p.published === '1' || 
                        String(p.published).toLowerCase() === 'true';
    
    // Log each product's published status to debug
    console.log(`Product ${p.id}: '${p.title}', published value: ${p.published}, isPublished: ${isPublished}`);
  ...
  const totalPublished = computed(() => {
    ...
    return products.value.data.filter(p => normalizePublished(p.published)).length;
  });
  ...
  // Don't add published:true to the search string
  // The API might not support this filtering syntax
  store.dispatch('getProducts', {
    force: true,
    search: search.value, // Remove the published:true filter from here
