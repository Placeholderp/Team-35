<template>
                ...
                <div class="relative">
                  ...
                  <form @submit.prevent="onSubmit">
                    <div class="p-6 space-y-4">
                      ...
                      <div v-if="forceChange" class="text-orange-600 p-2 bg-orange-50 rounded-md text-sm">
  ...
  const passwordMatch = computed(() => {
    if (!passwordData.value.confirmPassword) return null
  ...
  function closeModal() {
    if (props.forceChange) return // Prevent closing if change is forced
  ...
  function onSubmit() {
    ...
    const changePasswordData = {
      ...
      new_password_confirmation: passwordData.value.confirmPassword,
      user_id: props.userId
    }
    
    // Call store action to change password
    store.dispatch('changePassword', changePasswordData)
      .then(response => {
        loading.value = false
  ...
  // If forceChange is enabled, watch for changes to prevent closing
  watch(() => props.forceChange, (newValue) => {
    if (newValue) {
      // User cannot close the modal when forceChange is true
      show.value = true
    }
  })
  </script>